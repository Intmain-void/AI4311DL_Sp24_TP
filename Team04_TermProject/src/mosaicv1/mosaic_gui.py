...
#         self.registered_faces = []
#         self.selected_registered_faces = []  # List to store selected registered faces
#         self.selected_unregistered_faces = (
#             []
#         )  # List to store selected unregistered faces
#         self.matched_faces = {}  # Dictionary to store matched faces between lists
#         self.mosaic_factor = 1.0
...
#         matches = 0
#         self.matched_faces = {}  # Reset matched faces
#         # for reg_index, reg_enc in enumerate(registered_encodings):
#         #     for unreg_index, unreg_enc in enumerate(unregistered_encodings):
#         #         match = face_recognition.compare_faces(
#         #             [reg_enc], unreg_enc, tolerance=0.4
#         #         )
#         #         if match[0]:
#         #             self.matched_faces.setdefault(reg_index, []).append(unreg_index)
#         #             self.matched_faces.setdefault(unreg_index, []).append(reg_index)
#         #             matches += 1
#         for unreg_index, unreg_enc in enumerate(unregistered_encodings):
#             match = face_recognition.compare_faces(
#                 registered_encodings, unreg_enc, tolerance=0.6
#             )
#             for idx, same in enumerate(match):
#                 if same and 
#                 self.matched_faces.setdefault(idx, []).append(unreg_index)
#                 self.matched_faces.setdefault(unreg_index, []).append(idx)
#                 matches += 1
...
#                 # Sync selection with matched registered faces
#                 if index in self.matched_faces:
#                     for reg_index in self.matched_faces[index]:
#                         widget = self.registered_scrollable_frame.grid_slaves(
#                             row=reg_index // 3, column=reg_index % 3
#                         )[0]
#                         widget.select()
